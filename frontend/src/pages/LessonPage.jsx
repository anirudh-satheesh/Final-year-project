...
const LessonPage = () => {
  ...
  const handleMarkComplete = async () => {
    ...
    if (!completed.includes(topicId)) {
      ...
      localStorage.setItem('kaizen_completed_topics', JSON.stringify(completed));
    }
    ...
    if (savedJourneys) {
      ...
      const currentRoadmap = localStorage.getItem('kaizen_current_roadmap');
      if (currentRoadmap) {
        const roadmap = JSON.parse(currentRoadmap);
        // If the topic is in the current roadmap, update that journey
        const journeyTitle = localStorage.getItem('kaizen_current_subject'); // I should probably set this in App.jsx
